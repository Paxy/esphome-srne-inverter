...
| `number.max_charge_current` | (slow-poll pending) | `0xE205` | 0-200 A — manual §5.2 item 07 "Battery charge current". **Note: V1.7 PDF labels this register `mains_charge_current_limit`, but Anenji firmware actually exposes the battery-side limit here.** |
| `number.mains_charge_current_limit` | (slow-poll pending) | `0xE20A` | 0-100 A — manual §5.2 item 28 "Mains charge current". **Note: V1.7 PDF labels this register `maximum_charge_current` with max 150, but Anenji firmware actually exposes the mains-side cap and rejects >100 A here.** |
